...
namespace ASTImpl {
    ...
    namespace {
        ...
        public:
            ...
            ExprPrecedence GetPrecedence() const override {
                ...
                default:
                    ...
                    assert(false);
                    return static_cast<ExprPrecedence>(std::numeric_limits<int>::max());
                }
            }
        ...
        public:
            ...
            double Evaluate(const SheetInterface& args) const override {
                if (!cell_->IsValid()) {
                    throw FormulaException("Invalid position: " + cell_->ToString());
                }
                ...
                const auto& temp = cell->GetValue();
                switch (temp.index()) {
                case 0: {
                    const auto& temp_string = std::get<std::string>(temp);
                    if (temp_string.empty()) {
                        return 0.0;
                    }
                    try {
                        double value;
                        std::istringstream input(temp_string);
                ...
                case 1:
                    return std::get<double>(temp);
                case 2:
                    throw std::get<FormulaError>(temp);
                default:
                    assert(false);
                }
            }
